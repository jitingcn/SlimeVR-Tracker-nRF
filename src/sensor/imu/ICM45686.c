...
int icm45_init(float clock_rate, float accel_time, float gyro_time, float *accel_actual_time, float *gyro_actual_time)
{
	...
	err |= ssi_reg_write_byte(
		...
		0x80 | 0b000111
	); // set FIFO stop-on-full mode, set FIFO depth to 2K bytes (see AN-000364)
...
int icm45_update_odr(float accel_time, float gyro_time, float *accel_actual_time, float *gyro_actual_time)
{
	...
	if (last_accel_odr == 0xff || last_gyro_odr == 0xff || (last_accel_odr == 0 ? 0 : 1) != (ACCEL_ODR == 0 ? 0 : 1)
		|| (last_gyro_odr == 0 ? 0 : 1)
			   != (GYRO_ODR == 0 ? 0 : 1)) { // TODO: can't tell difference between gyro off and gyro standby
		err |= ssi_reg_write_byte(
			SENSOR_INTERFACE_DEV_IMU,
			ICM45686_PWR_MGMT0,
